...
for (ad in 1:4){
    ...
    for (subj in subjects){
        ...
        load (file.path (sprintf(paste("../Sujeito%d",addresses[ad],"grouped_left.dat",sep=""), subj)))
        left <- dwt.coefs

        load (file.path (sprintf(paste("../Sujeito%d",addresses[ad],"grouped_right.dat",sep=""), subj)))
        right <- dwt.coefs

        ## ROI selection

        dwt.coefs.left <- left [, c (1, idx)]
        dwt.coefs.right <- right [, c (1,idx)]

        stimidx <- dwt.coefs.left [, 1]

###normalize by the root sum squares (sd) or randomize
        if (sqr.rand==1){
            fname="-sqrt"
            dwt.coefs.left <- (dwt.coefs.left-mean(dwt.coefs.left))/(sqrt(sum(dwt.coefs.left^2)))
            dwt.coefs.right <- (dwt.coefs.right-mean(dwt.coefs.right))/(sqrt(sum(dwt.coefs.right^2)))
        }
        else if (sqr.rand==2){
            fname="-shuff"
            dwt.coefs.left <- dwt.coefs.left[sample(nrow(dwt.coefs.left)),]
            dwt.coefs.right <- dwt.coefs.right[sample(nrow(dwt.coefs.right)),]
        }
        else{
            fname=""
        }
...
###############################################################################################        
        ...
        for (i in seq (1, length (alphas))) {
            psy.EN.left [[i]] <- cv.glmnet (dwt.coefs.left [, 2 : ncol (dwt.coefs.left)],
                                            response1,
                                            nfolds = nstim,
        ...
        fit <- glmnet(dwt.coefs.left [, 2 : ncol (dwt.coefs.left)], response1, lambda=psy.EN.left.best$lambda.min)
...
##################################################################################################
        psy.EN.right <- list ()
        for (i in seq (1, length (alphas))) {
            psy.EN.right [[i]] <- cv.glmnet (dwt.coefs.right [, 2 : ncol (dwt.coefs.right)],
                                             response1,
                                             nfolds = nstim,
                                             foldid = rep (seq (1, groups), nstim),
                                             alpha = alphas [i])
        ...
        fit <- glmnet(dwt.coefs.right [, 2 : ncol (dwt.coefs.right)], response1, lambda=psy.EN.right.best$lambda.min)
...
################################################################################################################
        phy.EN.left <- list ()
        for (i in seq (1, length (alphas))) {
            phy.EN.left [[i]] <- cv.glmnet (dwt.coefs.left [, 2 : ncol (dwt.coefs.left)],
                                            response2,
                                            nfolds = nstim,
                                            foldid = rep (seq (1, groups), nstim),
                                            alpha = alphas [i])
        ...
        fit <- glmnet(dwt.coefs.left [, 2 : ncol (dwt.coefs.left)], response2, lambda=phy.EN.left.best$lambda.min)
...
####################################################################################################
        phy.EN.right <- list ()
        for (i in seq (1, length (alphas))) {
            phy.EN.right [[i]] <- cv.glmnet (dwt.coefs.right [, 2 : ncol (dwt.coefs.right)],
                                             response2,
                                             nfolds = nstim,
                                             foldid = rep (seq (1, groups), nstim),
                                             alpha = alphas [i])
        ...
        fit <- glmnet(dwt.coefs.right [, 2 : ncol (dwt.coefs.right)], response2, lambda=phy.EN.right.best$lambda.min)
...
###Input is a matrix where each line is a trial with a scalar response.
        proj.phy.left <- predict (phy.EN.left.best, dwt.coefs.left[, 2 : ncol (dwt.coefs.left)], s="lambda.min")
        proj.phy.right <- predict (phy.EN.right.best, dwt.coefs.right[, 2 : ncol (dwt.coefs.right)], s="lambda.min")

        proj.psy.left <- predict (psy.EN.left.best, dwt.coefs.left[, 2 : ncol (dwt.coefs.left)], s="lambda.min")
        proj.psy.right <- predict (psy.EN.right.best, dwt.coefs.right[, 2 : ncol (dwt.coefs.right)], s="lambda.min")
...
### Fisher analysis
        ...
        dev.copy2pdf(file = file.path (subj.dir, sprintf("Proj-left%s.pdf",fname)))
########################################################################################################
...
### Fisher analysis
        ...
        dev.copy2pdf(file = file.path (subj.dir, sprintf("Proj-right%s.pdf",fname)))
...
##########################################################################################################
        ...
        save(phy.EN.right.best, psy.EN.right.best, phy.EN.left.best, psy.EN.left.best, coef.phy.left, coef.phy.right, coef.psy.left, coef.psy.right, mb.l, mb.r, mp.l, mp.r, file=file.path(subj.dir, sprintf("subject%02d%s.dat",subj,fname)))
        ...
        dev.new()
        scalogram(coef.phy, coef.psy, from.level, mw, sound)

        dev.copy2pdf(file = file.path (subj.dir, sprintf("Scalogram-subj%02d%s.pdf", subj,fname)))
##########################################################################################################    
    ...
    save(AIC.coef, coefs.phy, coefs.psy, mps.l, mps.r, mbs.l, mbs.r, file = file.path(sprintf("Todos/matrices-all-%s%s-%d-groups.dat",c("VOTpass","VOTact","FormPass","FormAct")[ad],fname, groups)))

    filename=sprintf("Todos/DWT-matrices-all-%s%s-%d-groups.mat",c("VOTpass","VOTact","FormPass","FormAct")[ad],fname,groups)
    writeMat(filename,coefs.phy=coefs.phy, coefs.psy=coefs.psy, AIC.coef=AIC.coef)

    source ("EN-histogram.r")

    histogram(coefs.phy, coefs.psy, mw, sound)

    dev.copy2pdf(file = file.path (sprintf("Todos/Histogram-%s%s.pdf", c("VOTpass","VOTact","FormPass","FormAct")[ad],fname)))

    graphics.off()
    
    source ("EN-histogram-abs.r")

    histogram(coefs.phy, coefs.psy, mw, sound)

    dev.copy2pdf(file = file.path (sprintf("Todos/Histogram-%s-abs%s.pdf", c("VOTpass","VOTact","FormPass","FormAct")[ad],fname)))

    graphics.off()

    source ("EN-histogram-avg.r")

    histogram(coefs.phy, coefs.psy, mw, sound)

    dev.copy2pdf(file = file.path (sprintf("Todos/Histogram-%s-avg%s.pdf", c("VOTpass","VOTact","FormPass","FormAct")[ad],fname)))
...
### Fisher analysis
    ...
    dev.copy2pdf(file = file.path (sprintf("Todos/Predictions-all-%s-left%s.pdf", c("VOTpass","VOTact","FormPass","FormAct")[ad],fname)))
###############################################################################################################
...
### Fisher analysis
    ...
    dev.copy2pdf(file = file.path (sprintf("Todos/Predictions-all-%s-right%s.pdf", c("VOTpass","VOTact","FormPass","FormAct")[ad],fname)))
