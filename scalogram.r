...
plot.scalogram <- function (dwt.obj,
    ...
    if (x.axis) {
        ...
        ## 7 is the correct number of coefs in the lower level
        ## Take the samples form the first coefficients took from the beginning
        ## and end parts of the scalogram due to threshold effects of the periodic DWT
        discount.ini <- round(((baseline.length/eeg.origfreq)-((samp.freq/7)/samp.freq))*samp.freq)

        plot (NA, NA, xlim = c (-discount.ini, (2 * length (dwt.obj@W [[1]])-discount.ini)) / samp.freq,
