...
### Indicates the amount of colors to be used
    num.col <- 50
...
### Places 1 where the lasso coefficients are different of zero
    ...
    sum.psy.left <- colSums(abs(psy.left))/nb.subjects

    sum.phy.left <- colSums(abs(phy.left))/nb.subjects

    sum.psy.right <- colSums(abs(psy.right))/nb.subjects

    sum.phy.right <- colSums(abs(phy.right))/nb.subjects

### Amount of colors necessary
    ma.list <- rep(list(max(max(sum.phy.left), max(sum.phy.right), max(sum.psy.left), max(sum.psy.right))),4)
    mi.list <- rep(list(min(min(sum.phy.left), min(sum.phy.right), min(sum.psy.left), min(sum.psy.right))),4)

    colors <- colorRampPalette (c ("white", "black")) (num.col)
#######################################################################################################
...
###Filling in the slots W and V for the physical response after unormalize data (not adding the average here)
    wp <- c(sum.phy.left, sum.phy.right)
...
###Filling in the slots W and V for the behavioral response after unormalize data (not adding the average here)
    wb <- c(sum.psy.left, sum.psy.right)
...
###Apply the coresponding shift to the last V level only
            ...
            for (w in 1:amount.squares){
                ...
                if (h == nb.dwt.levels){
                    ...
                    if(obj.list[[c]]@V[[h]][w] != 0){    
                        ...
                        idx <- round(((obj.list[[c]]@V[[h]][w]-mi.list[[c]])*(num.col-1)/(ma.list[[c]]-mi.list[[c]]))+1)

                        if (idx<1 || obj.list[[c]]@V[[h]][w]<0.001){
                            polygon (x, y, col="white", border="grey")
                        }
                        else if (idx>num.col){
                            polygon (x, y, col=colors[num.col], border="grey")
                        }
                        else{
                ...
                if (obj.list[[c]]@W[[h]][w] != 0){
                    ...
                    idx <- round(((obj.list[[c]]@W[[h]][w]-mi.list[[c]])*(num.col-1)/(ma.list[[c]]-mi.list[[c]]))+1)  

                    if (idx<1 || obj.list[[c]]@W[[h]][w]<0.001){
                        polygon (x, y+1, col="white", border="grey")
                    }
                    else if (idx>num.col){
                        polygon (x, y+1, col=colors[num.col], border="grey")
                    }
                    else{
        ...
        axis(4,at=seq(0, (from.level+hgt), l=10),pos=c(last.level.length+2,0),labels=round(seq(0,ma.list[[c]],l=10),4),lwd = 0,las=1)
